...
export async function sha256HexWithPrefix(
  data: ArrayBuffer | Uint8Array,
): Promise<string> {
  return `0x${await sha256Hex(data)}`;
}

export async function hashCanonicalJson(value: unknown): Promise<string> {
  return sha256HexWithPrefix(
    new TextEncoder().encode(JSON.stringify(sortObjectKeys(value))),
  );
}
